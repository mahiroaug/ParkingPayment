...
async function getParkingStatusByAddress(user_addr) {
  try {
    ...
    console.log(
      `getParkingStatusByAddress::parkingPayment.parkingStatus : user_addr=${user_addr}`
    );
    const result = await parkingPayment_alc.methods
      .parkingStatus(user_addr)
      .call();
...
async function getBalanceInParkingPaymentByUserAddress(user_addr, token_addr) {
  try {
    ...
    console.error(
      `getBalanceInParkingPaymentByUserAddress:: Error: ${error.message}`
    );
  }
}
...
async function getDesignatedParkingOwnerByUserAddress(user_addr) {
  try {
    ...
    console.error(
      `getDesignatedParkingOwnerByUserAddress:: Error: ${error.message}`
    );
  }
}
